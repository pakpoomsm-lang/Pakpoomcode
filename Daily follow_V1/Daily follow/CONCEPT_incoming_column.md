...
## 8. คำถามที่ต้องให้ผู้ใช้ตัดสินใจก่อนลงมือ
1. **granularity** — เอาระดับ lot รวม (ก) หรือ match รายตัว item (ข)?
   *(แนะนำ ก ก่อน)*
2. **ตัวเลขที่อยากเห็นในคอลัมน์** — จำนวน "รายการ part" หรือ "ยอดรวม qty"
   หรือทั้งคู่?
3. **เกณฑ์สี** — ให้ 🟡/🔴 อิงอะไร (เช่น เข้าช้ากว่า `due_date`, หรือยอดไม่ครบ)
   หรือเอาแค่ 🟢 มี / ⚪ ไม่มี ก่อน?
4. **path `incoming.db`** บนเครื่องจริง (network share ตัวไหน) เพื่อตั้ง default
