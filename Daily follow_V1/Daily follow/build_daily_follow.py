...
<body>
  ...
  <script>
    ...
    function computeUnitRate(row) {{
      ...
      const v = (num(ts) / 0.5916) * num(stock);
      if (!v) return '';
    ...
    function updateSimReadout() {{
      ...
      for (const id of selected) {{
        ...
        rate += (num(row.ts) / 0.5916) * num(row.assyOrder);
    ...
    function renderLeadStrip() {{
      ...
      track.className = 'lead-track';
      for (const l of lines) track.appendChild(buildChip(l));
      els.leadStrip.appendChild(track);
    }}
